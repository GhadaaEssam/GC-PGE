...
def get_train_edge(data_edge_index, train_anchor, pgb):
    # Use standard Python lists instead of the deprecated pandas .append()
    train_edges = []
    test_edges = []

    # Convert to a mathematical Set for instantly fast lookups (O(1) instead of O(N))
    train_anchor_set = set(train_anchor.values)

    for i in range(len(data_edge_index)):
        if(i % 1000 == 0):
            pgb.update(i / len(data_edge_index))
            
        node1 = data_edge_index.iloc[i, 0]
        node2 = data_edge_index.iloc[i, 1]

        if (node1 in train_anchor_set) and (node2 in train_anchor_set):
            train_edges.append(data_edge_index.iloc[i, :])
        elif (node1 in train_anchor_set) or (node2 in train_anchor_set):
            test_edges.append(data_edge_index.iloc[i, :])
            
    # Convert the lists back into pandas DataFrames at the very end
    train_edge_index = pd.DataFrame(train_edges, columns=data_edge_index.columns, dtype=int)
    test_edge_index = pd.DataFrame(test_edges, columns=data_edge_index.columns, dtype=int)
