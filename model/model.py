...
class OmicsFusion(nn.Module):
    def __init__(self, latent_dim=300, num_omics=4):
        super(OmicsFusion, self).__init__()
        # We keep the separate gate networks, but REMOVE the Sigmoid.
        # We just want raw scores here.
        self.gates = nn.ModuleList([
            nn.Linear(latent_dim, latent_dim)
            for _ in range(num_omics)
        ])
    ...
    def forward(self, z_rna, z_meth, z_cnv, z_snv):
        ...
        # 1. Calculate raw "trust scores" for each omics type
        raw_scores = [gate(z) for gate, z in zip(self.gates, z_list)]
        
        # 2. Stack the scores so they are lined up next to each other
        # Shape: [4 (omics), batch_size, 300 (features)]
        stacked_scores = torch.stack(raw_scores, dim=0)
        
        # 3. THE FIX: Apply Softmax across dimension 0 (the 4 omics layers)
        # This mathematically forces the 4 weights to sum to 1.0
        attn_weights = F.softmax(stacked_scores, dim=0)
        
        # 4. Stack the actual data, multiply by the new strict weights, and sum into one vector
        stacked_z = torch.stack(z_list, dim=0)
        fused = torch.sum(attn_weights * stacked_z, dim=0)
        
        # (Optional but recommended for interpretability: save the weights to the class instance
        #  so we can extract them later to show the doctors)
